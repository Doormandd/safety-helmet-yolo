...
def train_yolov8x_gpu():
    """训练YOLOv8x模型 (GPU模式)"""
    
    print("="*60)
    print("YOLOv8x 安全帽检测模型训练 (GPU模式)")
    print("="*60)
    ...
    # 配置参数 - GPU优化
    config = {
        'model': 'yolov8x.pt',  # 最大尺寸模型
        'data': 'dataset.yaml', # 使用相对路径，便于迁移
        'epochs': 100,
        'imgsz': 640,
        'batch': 16,     # GPU建议16或32，根据显存调整 (YOLOv8x模型较大，16比较稳妥)
        'device': '0',   # 指定使用第一块GPU
        'workers': 8,    # 增加数据加载线程
        'project': 'runs/train',
        'name': 'yolov8x_helmet_gpu',
        'exist_ok': True,
        'pretrained': True,
        'optimizer': 'auto',  # 让YOLO自动选择最佳优化器 (通常是SGD或AdamW)
        'lr0': 0.01,
        'momentum': 0.937,
        'weight_decay': 0.0005,
        ...
        'save': True,
        'save_period': 10,
        'cache': True,   # GPU服务器内存通常较大，开启缓存加速
        'amp': True,     # 开启自动混合精度训练 (显著减少显存占用并加速)
        'verbose': True,
        'plots': True,   # GPU环境通常可以生成图表
    }
